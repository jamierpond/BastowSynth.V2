...
juce::AudioProcessorValueTreeState::ParameterLayout BASAudioProcessor::createParameterLayout()
{
  std::vector <std::unique_ptr<juce::RangedAudioParameter>> params;
    ...
    std::string basegId = "gainId";
    std::string basegName = "GainName";
    std::string basefId = "freqId";
    std::string basefName = "FreqName";
    std::string basewtId = "waveTypeId";
    std::string basewtName = "WaveTypeName";

    for (int i = 0; i < 33; i++)
    {
        std::string gainId = basegId + std::to_string(i);
        std::string gainName = basegName + std::to_string(i);
        
        std::string freqId = basefId + std::to_string(i);
        std::string freqName = basefName + std::to_string(i);

        std::string wavetypeId = basewtId + std::to_string(i);
        std::string wavetypeName = basewtName + std::to_string(i);

        auto gainParameter = std::make_unique<juce::AudioParameterFloat>(
            gainId, gainName,
            juce::NormalisableRange<float>(-64.0f, -12.0f, 1.0),
            -64.0f
        );

        auto freqParameter = std::make_unique<juce::AudioParameterFloat>(
            freqId, freqName,
           juce::NormalisableRange<float>(-24.0f, +24.0f, 0.0),1.0f);
        
       // auto waveTypeParameter = std::make_unique<juce::AudioParameterInt>(
        //   wavetypeId, wavetypeName,1, 4, 1,"WaveType");
        auto waveTypeParameter = std::make_unique<juce::AudioParameterBool>(wavetypeId, wavetypeName, false);
    ...
    auto MasterGainParameter = std::make_unique<juce::AudioParameterFloat>(gainIdM, GainNameM,
    juce::NormalisableRange<float>(-60.0f, 6.0f, 1.0), -64.0f);
    // Component Volume Button1 - 3
    auto CVB1 = std::make_unique<juce::AudioParameterBool>(cvb1Id, CVB1Name, false);
    auto CVB2 = std::make_unique<juce::AudioParameterBool>(cvb2Id, CVB2Name, false);
    auto CVB3 = std::make_unique<juce::AudioParameterBool>(cvb3Id, CVB3Name, false);
...
void BASAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    {
        if ((voice = dynamic_cast<WavetableSynthesiserVoice*>(synthesiser.getVoice(i))))
        {

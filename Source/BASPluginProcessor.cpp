...
BASAudioProcessor::createParameterLayout() {
  ...
  for (int i = 0; i < 33; i++) {
    ...
    auto gainParameter = std::make_unique<juce::AudioParameterFloat>(
        gainId, gainName, juce::NormalisableRange<float>(-64.0f, -12.0f, 1.0),
        -64.0f);

    auto freqParameter = std::make_unique<juce::AudioParameterFloat>(
        freqId, freqName, juce::NormalisableRange<float>(-24.0f, +24.0f, 0.0),
        1.0f);
  ...
  auto MasterGainParameter = std::make_unique<juce::AudioParameterFloat>(
      masterGainParamId, GainNameM, juce::NormalisableRange<float>(-60.0f, 6.0f, 1.0),
      -64.0f);
...
void BASAudioProcessor::handleNoteOn(juce::MidiKeyboardState *, int midiChannel,
...
void BASAudioProcessor::handlePitchWheel(juce::MidiKeyboardState *,
                                         int midiChannel, int wheelValue) {

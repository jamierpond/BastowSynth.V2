...
FirstVCompartment::FirstVCompartment(BASAudioProcessor& p) :
    audioProcessor_(p),
    ...
    frequencySliders(numSliders)
{
  //  setWaveTypesForOscillators();
    ...
    for(auto i = 0; i < numSliders; ++i)
    {
        ...
        createSliderAttachment(audioProcessor_.tree, frequencyAttachments_[static_cast<std::vector<BastowFreqSlider>::size_type>(i)],"freqId" + std::to_string(static_cast<std::size_t>(i) ), frequencySliders[static_cast<std::vector<BastowFreqSlider>::size_type>(i)]);
       // createSliderAttachment(audioProcessor.tree, frequencyAttachments[0], freqId0, frequencySliders[0]);
       // createSliderAttachment(audioProcessor.tree, frequencyAttachments[0], freqId0, frequencySliders[0]);
